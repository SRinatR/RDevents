...
export function AdminSupportComposer({ threadId, locale, onSent }: Props) {
  ...
  async function handleSend() {
    ...
    try {
      ...
      if (files.length > 0) {
        const uploaded = await adminSupportApi.uploadAttachments(threadId, files);
        attachmentIds = (uploaded.attachments as Array<{ id: string }>).map((a) => a.id);
      }
      const result = await adminSupportApi.reply(threadId, { body: body.trim(), attachmentIds });
      setBody('');
      setFiles([]);
      onSent(result.message);
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : '';
      setError(msg || (locale === 'ru' ? 'Не удалось отправить ответ.' : 'Failed to send reply.'));
    } finally {
      setSending(false);
    }
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
      ...
      <FieldTextarea
        ...
        placeholder={locale === 'ru'
          ...
          : 'Reply to user… (Ctrl+Enter to send)'}
        disabled={sending}
      />
