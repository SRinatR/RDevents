...
import { formatEventDateRange } from './dashboard.formatters';
  ...
  return (
    <div className="active-event-card">
      ...
      <div className="active-event-actions">
        <Link href={`/${locale}/cabinet/events/${event.slug}`} className="btn btn-primary">
          {locale === 'ru' ? 'Открыть мероприятие' : 'Open event'}
        </Link>
      </div>
    </div>
  );
