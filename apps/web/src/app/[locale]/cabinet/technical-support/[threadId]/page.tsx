...
import { technicalSupportApi } from '@/lib/api';
...
export default function SupportThreadPage({ params }: { params: Promise<{ threadId: string }> }) {
  ...
  const fetchThread = useCallback(
    async (silent = false) => {
      ...
      try {
        const result = await technicalSupportApi.getThread(threadId);
        setThread(result.thread as SupportThreadDetail);
        technicalSupportApi.markRead(threadId).catch(() => {});
      } catch {
        if (!silent) {
          setError(
  ...
  async function handleDeleteCurrentThread() {
    ...
    setSuccessMessage('');
    try {
      await technicalSupportApi.deleteThread(thread.id);
      setSuccessMessage(locale === 'ru' ? 'Обращение удалено.' : 'Ticket deleted.');
      router.replace(`/${locale}/cabinet/technical-support`);
    } catch {
      setError(
      ...
      <PageHeader
        ...
        actions={
          <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
            ...
            <Link href={`/${locale}/cabinet/technical-support`} className="btn btn-secondary btn-sm">
