...
import { EmptyState, FieldInput, FieldSelect, FieldTextarea, LoadingLines, Notice, Panel, StatusBadge } from '@/components/ui/signal-primitives';
        ...
        {loadingData ? (
          ...
          <div className="admin-table-mobile-cards">
            <AdminDataTable minWidth={960}>
              <AdminDataTableHeader
                columns={[
                  { label: locale === 'ru' ? 'Название' : 'Name', width: '20%' },
                  { label: locale === 'ru' ? 'Ключ' : 'Key', width: '18%' },
                  { label: locale === 'ru' ? 'Тема' : 'Subject', width: '28%' },
                  { label: locale === 'ru' ? 'Статус' : 'Status', width: '12%' },
                  { label: locale === 'ru' ? 'Обновлено' : 'Updated', width: '10%' },
                  { label: locale === 'ru' ? 'Действия' : 'Actions', align: 'right', width: '12%' },
                ]}
              />
              <AdminDataTableBody>
                {templates.map((tmpl) => (
                  <AdminDataTableRow key={tmpl.id}>
                    <AdminDataTableCell><AdminTableCellMain title={tmpl.name} subtitle={tmpl.key} /></AdminDataTableCell>
                    <AdminDataTableCell truncate className="signal-muted">{tmpl.key}</AdminDataTableCell>
                    <AdminDataTableCell truncate>{tmpl.subject}</AdminDataTableCell>
                    <AdminDataTableCell><StatusBadge tone={toneByStatus[tmpl.status] ?? 'neutral'}>{tmpl.status}</StatusBadge></AdminDataTableCell>
                    <AdminDataTableCell className="signal-muted">{new Date(tmpl.updatedAt).toLocaleDateString()}</AdminDataTableCell>
            ...
            <AdminMobileList>
              {templates.map((tmpl) => (
                <AdminMobileCard
                  ...
                  badge={<StatusBadge tone={toneByStatus[tmpl.status] ?? 'neutral'}>{tmpl.status}</StatusBadge>}
                  meta={[
                    { label: locale === 'ru' ? 'Ключ' : 'Key', value: tmpl.key },
                    { label: locale === 'ru' ? 'Обновлено' : 'Updated', value: new Date(tmpl.updatedAt).toLocaleDateString() },
                  ]}
                  actions={
                    <>
                      <button className="btn btn-secondary btn-sm" onClick={() => startEdit(tmpl)}>{locale === 'ru' ? 'Редактировать' : 'Edit'}</button>
