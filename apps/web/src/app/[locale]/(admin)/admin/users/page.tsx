...
import { EmptyState, FieldInput, FieldSelect, LoadingLines, MetricCard, Notice, Panel, SectionHeader, StatusBadge } from '@/components/ui/signal-primitives';
    ...
    <div className="signal-page-shell admin-control-page">
      ...
      <Panel variant="elevated" className="admin-command-panel">
        ...
        >
          ...
          />
          <FieldSelect value={roleFilter} onChange={(e) => setRoleFilter(e.target.value)} className="admin-filter-select admin-toolbar-select">
            <option value="">{isRu ? 'Все роли' : 'All roles'}</option>
            <option value="USER">User</option>
            <option value="PLATFORM_ADMIN">Platform Admin</option>
            <option value="SUPER_ADMIN">Super Admin</option>
          </FieldSelect>
          <FieldSelect value={hasEventMembershipFilter} onChange={(e) => setHasEventMembershipFilter(e.target.value)} className="admin-filter-select admin-toolbar-select">
          ...
          {events.length > 0 && (
            <FieldSelect value={eventIdFilter} onChange={(e) => setEventIdFilter(e.target.value)} className="admin-filter-select admin-toolbar-select">
          ...
          <>
            <div className="admin-table-mobile-cards">
              <AdminDataTable minWidth={1240}>
                <AdminDataTableHeader
                  columns={[
                    { label: isRu ? 'Пользователь' : 'User', width: '20%' },
                    { label: isRu ? 'Системная роль' : 'System role', width: '12%' },
                    { label: isRu ? 'Город' : 'City', width: '10%' },
                    { label: isRu ? 'Зарегистрирован' : 'Registered', width: '10%' },
                    { label: isRu ? 'Последний вход' : 'Last login', width: '10%' },
                    { label: isRu ? 'Участий' : 'Events', width: '8%' },
                    { label: isRu ? 'Команд' : 'Teams', width: '8%' },
                    { label: isRu ? 'Провайдеры' : 'Providers', width: '14%' },
                    { label: isRu ? 'Действия' : 'Actions', align: 'right', width: '18%' },
                  ]}
                />
                <AdminDataTableBody>
                  {users.map((entry) => (
                    <AdminDataTableRow key={entry.id} onClick={() => handleRowClick(entry.id)}>
                      <AdminDataTableCell>
                        <div className="admin-user-cell">
                          ...
                          <AdminTableCellMain title={entry.name || '—'} subtitle={entry.email} />
                        </div>
                      </AdminDataTableCell>
                      <AdminDataTableCell>
                        <StatusBadge tone={toneByRole[entry.role] ?? 'neutral'}>{entry.role}</StatusBadge>
                      </AdminDataTableCell>
                      <AdminDataTableCell>{entry.city || '—'}</AdminDataTableCell>
                      <AdminDataTableCell className="signal-muted">{entry.registeredAt ? new Date(entry.registeredAt).toLocaleDateString() : '—'}</AdminDataTableCell>
                      <AdminDataTableCell className="signal-muted">{entry.lastLoginAt ? new Date(entry.lastLoginAt).toLocaleDateString() : '—'}</AdminDataTableCell>
                      <AdminDataTableCell>{entry.counts.eventMembershipsTotal}</AdminDataTableCell>
                      <AdminDataTableCell>{entry.counts.teamsTotal}</AdminDataTableCell>
                      <AdminDataTableCell>
              ...
              <AdminMobileList>
                {users.map((entry) => (
                  <AdminMobileCard
                    ...
                    badge={<StatusBadge tone={toneByRole[entry.role] ?? 'neutral'}>{entry.role}</StatusBadge>}
                    meta={[
                      { label: isRu ? 'Город' : 'City', value: entry.city || '—' },
                      { label: isRu ? 'Участий' : 'Events', value: entry.counts.eventMembershipsTotal },
                      { label: isRu ? 'Команд' : 'Teams', value: entry.counts.teamsTotal },
                      {
                        label: isRu ? 'Провайдеры' : 'Providers',
                        value: entry.accounts?.length
                          ? entry.accounts.map((account) => providerLabel[account.provider] ?? account.provider).join(', ')
                          : '—',
                      },
                    ]}
                    actions={
