...
export default function AdminSupportThreadPage({ params }: { params: Promise<{ threadId: string }> }) {
  ...
  const fetchThread = useCallback(
    async (silent = false) => {
      ...
      try {
        const result = await adminTechnicalSupportApi.getThread(threadId);
        setThread(result.thread);
        adminTechnicalSupportApi.markRead(threadId).catch(() => {});
      } catch {
        if (!silent) {
  ...
  async function handleDeleteCurrentThread() {
    ...
    try {
      ...
      router.replace(`/${locale}/admin/technical-support`);
    } catch {
      ...
      <PageHeader
        ...
        actions={
          <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
            ...
            <Link href={`/${locale}/admin/technical-support`} className="btn btn-secondary btn-sm">
