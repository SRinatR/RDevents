...
export async function sendPasswordResetCodeEmail(input: {
  ...
  const from = formatFromAddress(env.RESEND_FROM_NAME, env.RESEND_FROM_EMAIL);
  const subject = 'Код для сброса пароля в RDEvents';
  const text = buildPasswordResetCodeText(input.code, input.ttlMinutes);
  const html = buildPasswordResetCodeHtml(input.code, input.ttlMinutes);
...
function buildPasswordResetCodeText(code: string, ttlMinutes: number) {
  return [
    'RDEvents',
    '',
    'RU',
...
function buildPasswordResetCodeHtml(code: string, ttlMinutes: number) {
  return `
    <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #111827;">
      <h2 style="margin-bottom: 12px;">RDEvents</h2>
      <div style="margin-bottom: 24px;">
        <h3 style="margin-bottom: 8px;">RU</h3>
        <p style="margin: 0 0 12px;">Ваш код для сброса пароля:</p>
        <div style="font-size: 28px; font-weight: 700; letter-spacing: 8px; margin: 0 0 12px;">${code}</div>
        <p style="margin: 0 0 8px;">Код действует ${ttlMinutes} минут.</p>
      ...
      <div>
        <h3 style="margin-bottom: 8px;">EN</h3>
        <p style="margin: 0 0 12px;">Your password reset code:</p>
        <div style="font-size: 28px; font-weight: 700; letter-spacing: 8px; margin: 0 0 12px;">${code}</div>
        <p style="margin: 0 0 8px;">This code is valid for ${ttlMinutes} minutes.</p>
